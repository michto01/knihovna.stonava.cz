     ...
     <div x-on:click.away="open = false" class="relative" x-data="{ open: false }">
        ...
        >
            ...
            <svg width=".7em" height=".7em"  fill="currentColor" viewBox="0 0 20 20" x-bind:class="{'rotate-180': open, 'rotate-0': !open}" class="inline w-4 h-4 mt-1 ml-1 transition-transform duration-200 transform md:-mt-1" clip-rule="evenodd" fill-rule="evenodd">
              <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"></path></svg>
        </button>
        <div 
          x-show="open"
...
export default {
  ...
  methods: {
    locales (locale) {
      ...
      var current = languages[locale.split('-')[0]];
      return current;
    },
    localeChanged ($event) {
      this.currentLocale = $event.srcElement.dataset.locale
      this.$router.push({
